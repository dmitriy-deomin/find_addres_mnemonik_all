...
pub fn get_conf_text(cpu_count:String)->String{
    let test_info = format!("Настройки программы (-значение должно отделяться пробелом от описания)
0 -Глубина проверки аккаунтов (0-2147483647)
1 -Глубина проверки внутрених адресов (0-2147483647)
1 -Глубина проверки внешних публичных адресов (0-2147483647)
1 -Включает/выключает(1/0) проход по всем возможным комбинациям сид фразы -128 шт
12 -Длинна сид фразы (12,15,18,21,24) слова, 0-случайная длинна, 1-по возрастанию
0 -Количество логических ядер для работы 0 до {} (0 - режим теста с отображение всей инфы)",cpu_count);
    test_info.to_string()
}

pub fn get_text_text_info()->String{
    let test_info = "Для теста добавьте один или несколько из этих адресов
в режиме измерения скорости(0 количесто ядер процессора)
после 5 секунд после запуска подставляеться тестовый секретный ключ
